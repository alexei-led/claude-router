...
export function rewriteRequest(body, tier, config) {
  ...
  // A route's own effort also goes into per-turn control, which overrides the request's effort on the model.
  const routeEffort = route.effort ? clampEffort(route.effort, model.efforts) : null;
  if (Array.isArray(body.messages)) out.messages = adaptMessages(body.messages, model.features ?? [], routeEffort);
...
function adaptMessages(messages, features, routeEffort) {
  let out = messages;
  if (!features.includes('per-turn-control')) out = out.map(withoutTurnControl).filter(Boolean);
  else if (routeEffort && out.some((m) => turnEffortOf(m) && turnEffortOf(m) !== routeEffort))
    out = out.map((m) => (turnEffortOf(m) ? { ...m, output_config: { ...m.output_config, effort: routeEffort } } : m));
...
// Per-turn control is an `output_config` (the turn's effort) on a message, usually the system message after the prompt.
// A message that carried only that is dropped: the API rejects empty content.
function withoutTurnControl(message) {
  if (!message || !Object.hasOwn(message, 'output_config')) return message;
  const { output_config: _turn, ...rest } = message;
  return Array.isArray(rest.content) && rest.content.length === 0 ? null : rest;
}

function turnEffortOf(message) {
  return message?.output_config?.effort ?? null;
}
