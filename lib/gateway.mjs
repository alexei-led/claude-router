...
export function createGateway({
  ...
  return createServer((req, res) => {
    ...
    req.on('end', async () => {
      try {
        ...
        const side = Boolean(parsed) && isSideRequest(parsed, routingHints(req).requestClass);
        ...
        up = send(
          ...
          (upRes) => relay(upRes, res, { routed, session, turn, side, clientLeft: () => closedEarly }),
        );
        up.on('error', (error) => {
  ...
  function relay(upRes, res, { routed, session, turn, side, clientLeft }) {
    ...
    pipeline(upRes, res, (error) => {
      ...
      if (reader.stopReason === 'tool_use' && !side) activity.turnPaused(session);
