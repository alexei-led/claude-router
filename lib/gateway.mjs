...
import { adaptBetas } from './rewrite.mjs';
import { isSideRequest } from './router.mjs';
...
// The betas the routed model accepts: rewrite.mjs owns what each model takes.
function withSupportedBetas(headers, modelId, config) {
  if (typeof headers['anthropic-beta'] !== 'string') return;
  const kept = adaptBetas(headers['anthropic-beta'], modelId, config);
  ...
  return createServer((req, res) => {
    ...
    req.on('end', async () => {
      try {
        ...
        // Only the conversation itself waits for tool results; a side request on the same session says nothing about it.
        // Only the hint header decides here: by body shape a turn with thinking off looks like a side request, and
        // keeping a wait too long costs less than exiting under a permission prompt.
        const { requestClass } = routingHints(req);
        const side = Boolean(requestClass) && isSideRequest(parsed ?? {}, requestClass);
        if (turn && !side) activity.turnResumed(session);
        let routed = null;
        if (parsed && router.isRouted(parsed)) {
          if (turn) {
            try {
              ...
              onError(error);
              routed = router.fallback(parsed, session, routingHints(req)); // never forward the alias upstream
            }
        ...
        if (routed) withSupportedBetas(headers, routed.body.model, router.config);
        up = send(
  ...
  function relay(upRes, res, { routed, session, turn, clientLeft }) {
    ...
    pipeline(upRes, res, (error) => {
      ...
      if (upRes.statusCode >= 300) {
        if (routed) tryRecord(() => router.recordFailure(session, routed, upRes.statusCode));
        return;
      }
      const usage = reader.end();
      if (reader.stopReason === 'tool_use') activity.turnPaused(session);
      if (!routed || routed.auxiliary) return;
      tryRecord(() => router.recordResponse(session, routed.tier, usage, routed.effort));
    });
  }

  function tryRecord(record) {
    try {
      record();
    } catch (error) {
      onError(error);
