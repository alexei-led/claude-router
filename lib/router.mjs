...
export function emptyMemory() {
  return {
    ...
    models: {},
    state: null,
    // Set by a pinned turn: the route to decide the next prompt from.
    pin: null,
  };
}
...
export class Router {
  ...
  async route(body, { sessionId, requestClass = null, agentType = null, contextCompacted = null }) {
    ...
    if (auxiliary) decision = { tier: this.config.gateway.auxiliaryTier, reason: 'auxiliary', state: memory.state };
      ...
      // A pin serves its turn and the turn's tool calls; the next prompt is decided from the route before the pin.
      if (memory.pin) {
        memory.lastRoute = memory.pin.returnTo;
        facts.lastRoute = memory.pin.returnTo;
        memory.pin = null;
      }
      decision = await this.decideTurn(facts, memory);
    }
    const pinnedTurn = decision.reason === 'pinned';
    ...
    if (!auxiliary) {
      if (pinnedTurn) memory.pin = { returnTo: memory.lastRoute };
  ...
  async decideTurn(facts, memory) {
    ...
    // A typed /router:<tier> pins this turn. Claude Code no longer switches the model from the skill's frontmatter.
    if (TIERS.includes(facts.pin))
      return { tier: facts.pin, reason: 'pinned', state: { ...state, turn: state.turn + 1 } };
    let advice = null;
    ...
    // Shadow only: what following Jev's choice instead of staying would cost at list prices. The policy does not read
    // this record; the downgrade tax computes its own from the same function.
