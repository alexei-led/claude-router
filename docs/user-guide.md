...
| The status line shows `gateway off` after the next prompt         | The gateway cannot start, for example because `router.json` has an unknown key. | Run `node <plugin>/scripts/ensure-gateway.mjs` to see the error. Read `gateway.log` next to `decisions.jsonl`. |
